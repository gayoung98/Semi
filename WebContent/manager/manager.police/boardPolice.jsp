			...
			<div class="container">
				...
				<div class="content">
					...
					<c:choose>
						<c:when test="${i=='>'}">
							<a
								href="${pageContext.request.contextPath}/boardPolice.manager?currentPage=${navi[s.index-1]+1}&category=${category}&search=${search}">${i}</a>
						</c:when>
						<c:when test="${i=='<'}">
							<a
								href="${pageContext.request.contextPath}/boardPolice.manager?currentPage=${navi[s.index+1]-1}&category=${category}&search=${search}">${i}</a>
						...
						<c:otherwise>
							<a
								href="${pageContext.request.contextPath}/boardPolice.manager?currentPage=${i}&category=${category}&search=${search}">${i}</a>
						</c:otherwise>
					</c:choose>
				</c:forEach>
							</td>
						...
						<div class="search">
							<form action="${pageContext.request.contextPath}/boardPolice.manager" method="get" >
								<div class="input-group">
									<div class="select">
										<select class="custom-select" id="inputGroupSelect04" name="category"
											...
											<option value="contents">신고 내용</option>
											<option value="id">신고자 id</option>
										</select>
									</div>
