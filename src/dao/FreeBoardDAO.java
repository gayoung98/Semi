...
public class FreeBoardDAO {
		...
		try(Connection con = this.getConnection();
			...
			try(ResultSet rs = pstat.executeQuery();){
				...
				while(rs.next()) {
					...
					FreeBoardDTO dto = new FreeBoardDTO(seq,branch,writer,title,null,null,write_date,viewCount,0);		           
					list.add(dto);
				}
				return list;
			}
	...
	public List<FreeBoardDTO> getPageList(int startNum, int endNum, String category, String keyWord) throws Exception{ // 검색할 때, 게시글 범위 출력 +카테고리, 키워드 포함 
		String sql = "select * from "
				+ "(select "
				+ "    row_number() over(order by seq desc) rnum, "
				+ "    seq, "
		...
		try(Connection con = this.getConnection();
			...
			try(ResultSet rs = pstat.executeQuery();){
				...
				while(rs.next()) {
					int seq = rs.getInt("seq");
					String branch = rs.getNString("title");
					String writer =rs.getNString("writer");
					String title = rs.getNString("title");
					Date write_date =rs.getDate("write_date");
					int viewCount = rs.getInt("viewCount");
